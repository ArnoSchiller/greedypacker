...
def scoreCP(rect: FreeRectangle, item: Item, self) -> int:
    ...
    for itm in self.items:
        if (itm.x == rect.x+rect.width or
            ...
            perim += common_interval_length(itm.y, itm.y +
                                            itm.height, rect.y, rect.y+item.height)
        if (itm.y == rect.y+rect.height or
                item.y+itm.height == rect.y):
            perim += common_interval_length(itm.x,
                                            itm.x+itm.width, rect.x, rect.x+item.width)
